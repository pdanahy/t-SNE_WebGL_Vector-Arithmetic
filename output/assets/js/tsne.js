...
World.prototype.addEventListeners = function() {
  ...
  // Add keyboard zoom with up/down arrow keys
  window.addEventListener('keydown', function(e) {
    if (e.key === 'ArrowUp') {
      // Zoom in
      if (this.controls && this.controls.type === 'trackball') {
        this.controls.zoomCamera(0.1); // simulate scroll in
      } else if (this.controls && this.controls.type === 'fly') {
        this.camera.position.z -= 0.1;
      }
    } else if (e.key === 'ArrowDown') {
      // Zoom out
      if (this.controls && this.controls.type === 'trackball') {
        this.controls.zoomCamera(10.0); // simulate scroll out
      } else if (this.controls && this.controls.type === 'fly') {
        this.camera.position.z += 0.1;
      }
    }
...
Lasso.prototype.handleMouseMove = function(e) {
  ...
  if (!this.isLassoEvent(e)) return;
  var canvas = document.getElementById('pixplot-canvas');
...
Lasso.prototype.draw = function() {
  ...
  // Debug: Log lasso hull points
  console.log('[LASSO] Hull points:', this.points);
  // remove the old mesh
  this.removeMesh();
...
Lasso.prototype.getHull = function() {
  ...
  var hull = l.getHull();
  console.log('[LASSO] getHull input points:', this.points);
  return hull;
}
...
Lasso.prototype.getSelectedMap = function() {
  ...
  var scaleX = canvas.width / rect.width;
  var scaleY = canvas.height / rect.height;
  // Only include valid points with x and y
  var polygon = (this.points || []).filter(function(i) {
    return i && typeof i.x === 'number' && typeof i.y === 'number';
  }).map(function(i) {
    return [i.x, i.y]; // lasso points are already in canvas pixel coordinates
  });
  // Debug: Log the polygon used for selection
  console.log('[LASSO] Selection polygon (canvas pixels):', polygon);
  ...
  var maxDist = 2.0; // threshold distance

  var selected = {};
  for (var i=0; i<data.json.images.length; i++) {
    var cell = data.cells[i];
    var cellPos = new THREE.Vector3(cell.x, cell.y, cell.z || 0);
    // Frustum and distance check
    // if (!frustum.containsPoint(cellPos)) continue;
    // if (camPos.distanceTo(cellPos) > maxDist) continue;
    // Convert cell's world position to screen space (pixels)
    var screenPos = worldToScreenCoords({x: cell.x, y: cell.y, z: cell.z || 0});
    // Convert screen position to canvas pixel coordinates
    var canvasPos = {
      x: screenPos.x * scaleX,
      y: screenPos.y * scaleY
    };
    // Debug: Log each cell's world, screen, and canvas position
    // if (i < 10) {
    //   console.log(`[LASSO] Cell ${i}: world=`, {x: cell.x, y: cell.y, z: cell.z || 0}, 'screen=', screenPos, 'canvas=', canvasPos);
    // }
    // (frustum.containsPoint(cellPos)
    // if  (camPos.distanceTo(cellPos) < maxDist) {
    var inside = pointInPolygon([canvasPos.x, canvasPos.y], polygon);
    selected[data.json.images[i]] = inside;
    // }
...
Modal.prototype.showCells = function(cellIndices, cellIdx) {
  var self = this;
  // settings.close(); // Remove or comment out this line
...
Hotspots.prototype.render = function() {
  ...
  for (var i=0; i<hotspots.length; i++) {
    ...
    var imgElem = hotspots[i].querySelector('.hotspot-image');
    if (imgElem) {
      imgElem.addEventListener('click', function(idx) {
        world.flyToCellImage(this.json[idx].img);
      }.bind(this, i));
    }
    // show the number of cells in this hotspot's cluster
    var bar = hotspots[i].querySelector('.hotspot-bar-inner');
    if (bar) {
      bar.style.width = (100 * this.json[i].images.length / data.cells.length) + '%';
    }
